...
  location: string;
  currentStock: number;
  onMovementRegistered: () => void; // Callback para refrescar datos en la página de detalles
}
...
    formState: { errors },
    reset,
    watch, // Para observar el tipo de movimiento y la cantidad
  } = useForm<ProductMovementFormInputs>({
    defaultValues: {
      tipoMovimiento: 'Salida', // Valor por defecto
...
  // Resetear el formulario cuando el modal se cierra o abre
  useEffect(() => {
    if (open) {
      reset({
...
      onMovementRegistered(); // Llama al callback para refrescar
...
    if (!loading) {
      onClose(); // El reset se maneja en el useEffect de 'open'
    }
  };
