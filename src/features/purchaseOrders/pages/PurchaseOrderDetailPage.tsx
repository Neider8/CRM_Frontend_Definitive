...
  const [editingPurchaseDetail, setEditingPurchaseDetail] = useState<PurchaseOrderDetailDetails | null>(null);

  // Estado para el modal de edición de cabecera
  const [openEditHeaderModal, setOpenEditHeaderModal] = useState(false);
  const [openAddDetailModal, setOpenAddDetailModal] = useState(false); // Estado para el modal de añadir detalle
...
  const handleEditDetailClick = (detail: PurchaseOrderDetailDetails) => {
...
      {/* Modal para añadir detalle a la orden de compra */}
      {order && (
        <PurchaseOrderDetailAddModal
          open={openAddDetailModal}
          onClose={() => setOpenAddDetailModal(false)}
          orderId={order.idOrdenCompra}
          existingInsumoIdsInOrder={order.detalles?.map(d => d.insumo.idInsumo) || []}
          onDetailAdded={handleDetailAdded}
        />
      )}

      {/* Modal para editar detalle de la orden de compra */}
      {editingPurchaseDetail && (
        <PurchaseOrderDetailEditModal
          open={!!editingPurchaseDetail}
          onClose={() => setEditingPurchaseDetail(null)}
          orderId={order.idOrdenCompra}
          detailData={editingPurchaseDetail}
